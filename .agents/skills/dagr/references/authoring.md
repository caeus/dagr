...
`dagr:glob` exposes `of(pattern)`, which validates and compiles the pattern once and returns a reusable matcher with `match(value)`. `/` separates segments, `*` matches one segment, and `**` matches zero or more segments. Wildcards must occupy an entire segment. It only matches strings and never accesses the filesystem.
