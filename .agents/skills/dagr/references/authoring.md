...
`dagr:glob` exposes `match(pattern, value)`. `/` separates segments, `*` matches one segment, and `**` matches zero or more segments. Wildcards must occupy an entire segment. It only matches strings and never accesses the filesystem.
