...
- Add adapter bindings ending in `/materialized` for normal target file integration and `/hoisted` for host materialization. Keep each producer's canonical file binding separate from both adapters.
- Use `many()` only for explicit open protocols such as `/**/materialized`, `/**/hoisted`, `/**/package-json/script`, and `/**/tooling/for/<consumer>`.
...
- Keep package-manager behavior in explicit feature graphs such as `npm()`, `pnpm()`, and `yarn()`. A custom manager provides the exact `/package-manager/**` capabilities plus canonical config and its materialization adapters.
