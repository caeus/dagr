...
`.dagr` and `.git`. Mount requests stay opaque and their volumes are not built or extracted by
`dagr list`.
...
`.dagr` and `.git`, and leaves mount requests opaque, so mounted packages are not listed. When no
package lies at or under the working directory, the command prints nothing and exits `0`.
